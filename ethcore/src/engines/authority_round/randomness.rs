...
impl<'a> BoundContract<'a> {
    ...
    fn call_const<D>(&self, call: (ethabi::Bytes, D)) -> Result<D::Output, PhaseError> where
        D: ethabi::FunctionOutputDecoder
    {
        ...
        let call_return = self.client.call_contract(self.block_id, self.contract_addr, data).expect("FIXME");
        ...
        output_decoder.decode(call_return.as_slice()).map_err(PhaseError::MachineError)
    }
}
...
impl RandomnessPhase {
    ...
    fn load(contract_address: Address, our_address: Address) -> Result<RandomnessPhase, PhaseError> {
        let contract = BoundContract::bind(unimplemented!(), unimplemented!(), contract_address);
        ...
        let is_reveal_phase = contract.call_const(aura_random::functions::is_reveal_phase::call())?;
        let is_commit_phase = contract.call_const(aura_random::functions::is_commit_phase::call())?;
