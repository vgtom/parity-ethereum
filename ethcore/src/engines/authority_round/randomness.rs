...
pub enum PhaseError {
	...
	CallError(String),
	/// Decoding a called contract's return value failed while determinig the phase.
	DecodeError(ethabi::Error),
}


/// Utility structure for binding contracts.
///
...
impl<'a> BoundContract<'a> {
	...
	pub fn call_const<D>(&self, call: (ethabi::Bytes, D)) -> Result<D::Output, PhaseError>
	where
		D: ethabi::FunctionOutputDecoder,
	{
		...
		output_decoder
			...
			.map_err(PhaseError::DecodeError)
	}
}
