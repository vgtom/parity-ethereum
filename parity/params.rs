...
impl str::FromStr for SpecType {
	...
	fn from_str(s: &str) -> Result<Self, Self::Err> {
		let spec = match s {
			...
			"poacore" => SpecType::Poacore,
			"poasokol" => SpecType::Poasokol,
...
impl fmt::Display for SpecType {
	fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
		f.write_str(match *self {
			...
			SpecType::Poacore => "poacore",
			SpecType::Poasokol => "poasokol",
...
impl SpecType {
	pub fn spec<'a, T: Into<SpecParams<'a>>>(&self, params: T) -> Result<Spec, String> {
		...
		match *self {
			...
			SpecType::Poacore => Ok(ethereum::new_poacore(params)),
			SpecType::Poasokol => Ok(ethereum::new_poasokol(params)),
